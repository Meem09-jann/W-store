...
const NavigationItem: FC<NavigationItemProps> = ({ menuItem }) => {
  ...
  const renderMegaMenuNavlink = (item: NavItemType) => {
    return (
      <li key={item.catId} className={`${item.isNew ? "menuIsNew" : ""}`}>
        <Link
  ...
  const renderDropdownMenuNavlink = (item) => {
    return (
      <Link key={item.id}
        ...
        href={`/category/` + item.id + `/` + item.sub_cat_name + `?type=subcategory`}
      >
        {item.sub_cat_name}
        {item.type && (
          <ChevronDownIcon
  ...
  const renderMainItem = (item) => {
    return (
      <div className="h-20 flex-shrink-0 flex items-center" key={item.catId}>
        <Link
          className="inline-flex items-center text-sm lg:text-[12px] font-medium text-slate-700 dark:text-slate-300 py-2.5 px-4 xl:px-5 rounded-full hover:text-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-slate-200"
          //href=''
          href={`/category/` + item.catId + `/` + item.catName + `?type=category`}
        >
          {item.catName}
          {item.type && (
            <ChevronDownIcon
