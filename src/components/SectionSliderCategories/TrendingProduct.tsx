...
import ProductCard2 from "@/components/CardCategories/ProductCard2"
...
const ShopList = ({
  ...
  useEffect(() => {
    ...
    let slider = new Glide(relatedRef.current, OPTIONS);
    slider.mount();
  ...
  return (
    <div className={`nc-SectionSliderCategories`}>
      <div ref={relatedRef} className={`glidejs flow-root `}>
        ...
        <div className="glide__track" data-glide-el="track">
          <ul className="glide__slides">
            {data?.map((item, index) => (
              <li key={index} className={`glide__slide ${itemClassName} w-[250px]`}>
                ...
                <ProductCard2
                  ...
                  featuredImage={item.cover_image ? `${process.env.NEXT_PUBLIC_IMAGE_BASE}` + item.cover_image : defaultimg}
                  //featuredImage={`${process.env.NEXT_PUBLIC_IMAGE_BASE}`+item.photoURL}
                  name={item.name}
                  desc={item.total_view + ` Views`}
                  bgClass={item.color}
