...
interface ExploreType {
  ...
  catName: string;
  description: string;
  catIcon: string | StaticImageData;
  svgBg: string;
  color: "bg-blue-50";
  //count?: number;
}
...
const SectionGridMoreExplore: FC<SectionGridMoreExploreProps> = ({
  ...
  const renderCard = (item: ExploreType) => {
    switch (boxCard) {
      case "box1":
        return (
          <CardCategory1 key={item.catId} featuredImage={`${process.env.NEXT_PUBLIC_IMAGE_BASE}`+item.catIcon} {...item} />
        );
      ...
      case "box4":
        return (
          <CardCategory4
            bgSVG={item.svgBg}
            featuredImage={`${process.env.NEXT_PUBLIC_IMAGE_BASE}`+item.catIcon}
            key={item.catId}
